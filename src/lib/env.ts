...
export function validateEnv(): ValidationResult {
  ...
  if (!isProd) {
    if (!process.env.JWT_SECRET) {
      warnings.push('JWT_SECRET not set, using random secret (tokens will not persist across restarts)');
    }
  }

  // API Key 配置警告
  if (!process.env.OPENROUTER_API_KEY && !process.env.ALLOWED_API_KEYS) {
    warnings.push('Neither OPENROUTER_API_KEY nor ALLOWED_API_KEYS configured. Public mode will require users to provide their own API keys.');
  }
